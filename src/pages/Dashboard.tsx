...
export const Dashboard = () => {
  ...
  // Weekly Check Stats
  const checkedThisWeekCount = inventory.filter(inv => {
    if (!inv.lastCheckedAt) return false;
    return new Date(inv.lastCheckedAt) >= thisWeekStart;
  }).length;

  const uncheckedCount = totalItemsCount - checkedThisWeekCount;
  const isCheckedThisWeek = totalItemsCount > 0 && uncheckedCount === 0;

  const discrepancyCountThisWeek = auditLogs.filter(log => {
    return log.action === 'check' && log.quantityChange !== 0 && new Date(log.timestamp) >= thisWeekStart;
  }).length;

  const weeklyCheckProgress = totalItemsCount > 0 ? Math.round((checkedThisWeekCount / totalItemsCount) * 100) : 0;
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 max-w-5xl mx-auto pb-24 md:pb-8">
      ...
      {/* TOP HERO CARD - WEEKLY CHECK STATUS */}
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className={`px-2.5 py-1 text-xs font-bold uppercase rounded-md ${isCheckedThisWeek ? 'bg-success/10 text-success' : 'bg-warning/10 text-warning-700'}`}>
                {isCheckedThisWeek ? 'Completed' : 'Pending'}
              </span>
              <span className="text-sm font-bold text-slate-500">This Week's Check</span>
            </div>
            <h2 className="text-3xl font-bold text-slate-800 mb-1">
              Central ED Inventory
            </h2>
            <p className="text-slate-500">
              {checkedThisWeekCount} of {totalItemsCount} medications verified.
            </p>
            
            {!isCheckedThisWeek && totalItemsCount > 0 && (
              <div className="mt-4 flex items-center gap-3 max-w-sm">
                <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                  <div className="h-full bg-primary-500 rounded-full" style={{ width: `${weeklyCheckProgress}%` }} />
                </div>
                <span className="text-xs font-bold text-slate-400">{weeklyCheckProgress}%</span>
              </div>
            )}
          </div>
          ...
          <div className="w-full md:w-auto shrink-0">
            {isCheckedThisWeek ? (
              ...
              >
                {checkedThisWeekCount > 0 ? 'Continue Check' : 'Start Weekly Check'}
              </button>
            )}
          </div>
        </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="card p-4 bg-white border-l-4 border-amber-500 hover:shadow-md transition-shadow">
            <p className="text-sm font-medium text-slate-500 mb-1">Unchecked This Week</p>
          ...
          <div className="card p-4 bg-white border-l-4 border-rose-500 hover:shadow-md transition-shadow cursor-pointer" onClick={() => navigate('/logs')}>
            ...
            <p className="text-2xl font-bold text-slate-800">{discrepancyCountThisWeek}</p>
          </div>
        ...
        <div className="md:col-span-1 space-y-3">
          ...
          <Link to="/check" className="flex items-center justify-between p-4 bg-white rounded-xl shadow-sm border border-slate-200 hover:border-primary-300 hover:bg-primary-50 transition-colors group">
            <div className="flex items-center gap-3">
              ...
              <span className="font-bold text-slate-700">Weekly Check</span>
            </div>
